...
<div class="form">
    ...
    <form class="formulario" action="#" method="post">
        <div class="nome_email">
            <input type="text" name="nome" placeholder="Seu nome e sobrenome">
            <input type="email" name="email" placeholder="Seu email">
        </div>
        <div class="msg">
            <textarea name="msg_texto" id="" cols="30" rows="8" placeholder="Sua mensagem"></textarea>
        </div>
        <div class="btn">
    ...
    if (count($_POST) > 0) {
        ...
        $nome = $_POST['nome'];
        $email = $_POST['email'];
        $texto = $_POST['msg_texto'];

        $sql = $conn->query("INSERT INTO contato(nomes, emails, mensagens) VALUES ('$nome', '$email', '$texto')");
        if ($sql) {
            print "<script>alert('Dados enviados com sucesso')</script>";
        }else{
            print "<script>alert('Houve um problema para enviar os dados');
            console.log('Mano deu algum problema ai " . $conn->error ."')
            </script>";
        }
    }
    ?>
